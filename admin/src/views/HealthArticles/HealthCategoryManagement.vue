...
import {
  ...
  deleteHealthCategoryApi,
  createHealthCategoryApi,
  updateHealthCategoryApi,
  type HealthCategory
...
const { tableRegister, tableState, tableMethods } = useTable({
  fetchDataApi: async () => {
    ...
    const res = await getHealthCategoryListApi({
      page: unref(currentPage),
      pageSize: unref(pageSize),
      ...unref(searchParams)
    })

    return extractPagedTableData<HealthCategory>(res)
  }
})
...
const currentRow = ref<HealthCategory | null>(null)
const writeRef = ref()
...
const handleEdit = (row: HealthCategory) => {
  dialogTitle.value = '编辑分类'
  currentRow.value = row
  dialogVisible.value = true
}

const handleDelete = async (row: HealthCategory) => {
  try {
    ...
    await deleteHealthCategoryApi(row.id)
...
const handleSave = async () => {
  ...
  try {
    ...
    if (currentRow.value) {
      await updateHealthCategoryApi(currentRow.value.id, formData)
      ElMessage.success('更新成功')
    } else {
      await createHealthCategoryApi(formData)
