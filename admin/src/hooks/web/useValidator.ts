...
export const useValidator = () => {
  /**
   * 必填验证
   * @param options - 可选的配置项或消息字符串
   * @param options.message - 验证失败时的提示消息
   * @param options.trigger - 触发验证的时机
   */
  const required = (options?: string | { message?: string; trigger?: string }): FormItemRule => {
    if (typeof options === 'string') {
      return {
        required: true,
        message: options || t('common.required')
      }
    }
    return {
      required: true,
      message: options?.message || t('common.required'),
