...
import { TCM_DIAGNOSIS_QUEUE } from '../queues/tcm-diagnosis.queue';
 ...
 * 调用第三方中医诊断接口并保存结果
 */
@Processor(TCM_DIAGNOSIS_QUEUE)
export class TcmDiagnosisProcessor {
  private readonly logger = new Logger(TcmDiagnosisProcessor.name);
  ...
  async handleDiagnosis(job: Job<{ reportId: number; symptoms: string }>) {
    ...
    this.logger.log(`开始处理中医诊断任务: reportId=${reportId}`);

    // 记录任务开始
    await this.aiDiagnosisLogger.logInfo(reportId, 'TCM', '中医诊断任务开始', {
      jobId: job.id,
      symptoms:
    ...
    try {
      ...
      const apiUrl = `${baseUrl}/api/v1/vet/herb`;
      ...
      const data = await requestDiagnosis(
        ...
        async (progress) => {
          await this.reportRepository.update(reportId, {
            tcmDiagnosis: progress,
          });
        },
        previous?.tcmDiagnosis,
      );

      const duration = Date.now() - startTime;

      this.logger.log(`中医诊断成功: reportId=${reportId}, 耗时=${duration}ms`);
      ...
      // 更新报告状态和结果
      await this.reportRepository.update(reportId, {
        tcmDiagnosis: diagnosisResult(data, false), // 保留增量评估和接口声明
        tcmJobId: job.id.toString(),
      });
      ...
      const duration = Date.now() - startTime;
      this.logger.error(
        `中医诊断失败: reportId=${reportId}, error=${error.message}`,
      );
      ...
      // 标记中医诊断失败
      await this.markTcmFailed(reportId, error.message);
   ...
   */
  private async markTcmFailed(reportId: number, errorMessage: string) {
    try {
      this.logger.log(`开始标记中医诊断失败: reportId=${reportId}`);
      ...
      this.logger.error(
        `✅ 中医诊断失败，报告已标记为失败: reportId=${reportId}, status=${updates.status}, reason=${updates.failureReason}`,
      );
    } catch (error) {
      this.logger.error(
        `❌ 标记中医诊断失败时出错: reportId=${reportId}, error=${error.message}`,
        error.stack,
      );
    }
  }
